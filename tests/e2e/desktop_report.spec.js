...
test.describe('synthetic desktop audit report workflow', () => {
  test('desktop workflow commits reviewed synthetic findings into the existing heatmap', async ({ page }) => {
    await page.addInitScript(() => {
      ...
      const finding = (id, title) => ({ finding_id:id, title, fact_summary:`${id} 虚构单位甲，金额 ${id === 'F-2' ? '12' : '8'} 万元`, source_page:'第 1 页', source_excerpt:`${id} 合成摘录`, matched_risk_id:'R001', domain:'采购与外包', likelihood:3, impact_scores:Object.fromEntries(dims.map(d => [d, 2])), rationale:`${id} 独立维度证据`, needs_review:true, review_status:'待确认', merged_finding_ids:[], merged_into:'' });
      let findings = [finding('F-1','第一项'), finding('F-2','第二项'), finding('F-3','第三项'), {...finding('F-4','新增风险项'),matched_risk_id:''}];
      let poll = 0;
      let startCalls = 0;
      let workbookCalls = 0;
      let loadCalls = 0;
      ...
      window.pywebview = { api: {
        ...
        start_analysis: async (reportToken, workbookToken, selectedPeriod) => { if(reportToken !== 'report' || workbookToken !== 'BOOK-A' || selectedPeriod !== '2026H2') throw {code:'BINDING_INVALID'}; startCalls++; await new Promise(resolve => setTimeout(resolve, 80)); return { task:{task_id:'T-1',status:'提取中',extraction_method:'text'}, period:selectedPeriod, risk_catalog:[{risk_id:'R001',period:'2026H2',owner_dept:'信息科技部'}] }; },
        ...
        merge_findings: async (_task, ids, payload) => { if(dims.some(dim => Object.hasOwn(payload, dim)) || Object.keys(payload.impact_scores || {}).length !== dims.length) throw {code:'INVALID_PAYLOAD'}; const changed = []; findings = findings.map(item => { const next = item.finding_id === ids[1] ? {...item,review_status:'已接受',merged_into:ids[0],merged_finding_ids:[]} : item.finding_id === ids[0] ? {...payload,finding_id:ids[0],merged_finding_ids:ids.slice(1),merged_into:''} : item; if(ids.includes(item.finding_id)) changed.push(next); return next; }); return {findings:changed}; },
        split_finding: async (_task, id, payloads) => ({findings:[{...findings.find(item => item.finding_id === id),review_status:'已排除'},...payloads]}),
        preview_commit: async (_task, workbookToken, selectedPeriod, decisions, stage, controlsConfirmed) => { if(stage === 'load_controls') { loadCalls++; await new Promise(resolve => setTimeout(resolve, 100)); return {controls_by_decision:decisions.map(item => ({...item,controls:item.action === 'merge' ? [{description:`CONTROL-${workbookToken}`,score:4,key:true}] : []}))}; } const included = decisions.filter(item => item.action !== 'exclude').flatMap(item => item.finding_ids); if(!controlsConfirmed || findings.some(item => included.includes(item.finding_id) && item.review_status !== '已接受')) throw {code:'PENDING_REVIEW'}; const merged=decisions.find(item=>item.action==='merge'), created=decisions.find(item=>item.action==='create'); if(workbookToken !== 'BOOK-A' || selectedPeriod !== '2026H2' || merged.finding_ids.join('|') !== 'F-1|F-2' || merged.owner_dept !== '信息科技部' || merged.remediation_status !== '整改中' || merged.domain !== '信息系统' || created.finding_ids.join('|') !== 'F-4' || created.owner_dept !== '运营部' || created.remediation_status !== '未整改' || !merged.controls.some(control => control.description === 'CONTROL-BOOK-A（已复核）' && control.score === 4 && control.key)) throw {code:'STALE'}; return {commit_token:'once',new_risks:[{risk_id:'R900'}],updated_risks:[{risk_id:'R001'}],new_controls:[],excluded_count:1,warnings:[]}; },
        commit_to_workbook: async (_task, _book, selectedPeriod, decisions) => { const merged=decisions.find(item=>item.action==='merge'), created=decisions.find(item=>item.action==='create'); if(merged.finding_ids.join('|')!=='F-1|F-2' || created.owner_dept!=='运营部') throw {code:'MERGE_IDS_LOST'}; return {workbook_path:'C:/synthetic/out.xlsx',export_dir:'C:/synthetic/export',period_data:{period:selectedPeriod,risks:[{risk_id:'R001',name:'确认风险',domain:'信息系统',description:'合成描述',owner_dept:merged.owner_dept,period:selectedPeriod,likelihood:3,...Object.fromEntries(dims.map(d => [d, 2])),rationale:'合成依据\n[整改状态：整改中]'},{risk_id:'R900',name:'新增风险项',domain:'采购与外包',description:'合成描述',owner_dept:created.owner_dept,period:selectedPeriod,likelihood:3,...Object.fromEntries(dims.map(d => [d, 2])),rationale:'合成依据\n[整改状态：未整改]'}],controls:[]}}; }
      }};
    });
    ...
    await expect(page.locator('[data-finding-id]')).toHaveCount(4);
    ...
    await page.locator('[data-finding-id="F-4"]').click();
    await page.getByRole('button', { name:'接受', exact:true }).click();
    await page.getByRole('button', { name:'进入风险评估' }).click();
    await expect(page.getByLabel('责任部门 F-1')).toHaveValue('信息科技部');
    await expect(page.getByLabel('责任部门 F-4')).toHaveValue('');
    await page.getByRole('button', { name:'载入控制点 / 生成预览' }).click();
    await expect(page.locator('#report-status')).toContainText('责任部门与整改状态');
    expect(await page.evaluate(() => window.__desktopLoadCalls())).toBe(0);
    await page.getByLabel('整改状态 F-1').selectOption('整改中');
    await page.getByLabel('责任部门 F-4').fill('运营部');
    await page.getByLabel('整改状态 F-4').selectOption('未整改');
    await page.getByRole('button', { name:'载入控制点 / 生成预览' }).click();
    await expect.poll(() => page.evaluate(() => window.__desktopLoadCalls())).toBe(1);
    await expect(page.getByLabel('控制点 F-1|F-2')).toHaveValue('CONTROL-BOOK-A');
    await page.screenshot({ path:'output/playwright/desktop-step4-controls-1440x920.png' });
    await page.getByLabel('控制点 F-1|F-2').fill('CONTROL-BOOK-A（已复核）');
    await page.getByLabel('控制点 F-1|F-2').press('Tab');
