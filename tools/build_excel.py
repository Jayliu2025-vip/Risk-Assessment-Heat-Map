...
def add_band_rules(ws, rng):
    """按阈值给连续数值区域上五档底色（阈值引用参数配置 B12:B15，可随参数变化）。"""
    first = rng.split(":")[0]
    rules = [
        ("extreme", f'AND(ISNUMBER({first}),{first}>=参数配置!$B$12)'),
        ("high", f'AND(ISNUMBER({first}),{first}>=参数配置!$B$13,{first}<参数配置!$B$12)'),
        ("medium", f'AND(ISNUMBER({first}),{first}>=参数配置!$B$14,{first}<参数配置!$B$13)'),
        ("low", f'AND(ISNUMBER({first}),{first}>=参数配置!$B$15,{first}<参数配置!$B$14)'),
        ("minimal", f'AND(ISNUMBER({first}),{first}<参数配置!$B$15)'),
    ]
...
def build_config_sheet(wb):
    ...
    dims = [("经济损失权重", "imp_financial"), ("合规法律权重", "imp_compliance"),
            ("运营中断权重", "imp_operation"), ("声誉舆情权重", "imp_reputation"),
            ("舞弊风险权重", "imp_fraud")]
    for i, (label, key) in enumerate(dims, start=3):
        ws[f"A{i}"] = label
        ws[f"B{i}"] = DEFAULT_CONFIG["weights"][key]
    ...
    ws.conditional_formatting.add(
        "B8", CellIsRule(operator="notEqual", formula=["1"],
                         fill=PatternFill("solid", fgColor="C00000"), stopIfTrue=True))
    ws["A9"] = "注：五个权重之和不为 1 时，合计单元格变红，导出脚本将拒绝执行。"
    ws["A9"].font = F_NOTE

    ws["A11"] = "剩余风险等级阈值（≥）"
    ws["A11"].font = F_SECTION
    for i, (label, val) in enumerate([("极高风险", 20), ("高风险", 12),
                                      ("中风险", 6), ("低风险", 3)], start=12):
        ws[f"A{i}"] = label
        ws[f"B{i}"] = val
        ws[f"B{i}"].fill = FILL_INPUT
    ...
    ws["A24"] = "分领域影响权重矩阵（各行列合计须=1；登记册领域未列出时自动使用“全领域默认”行）"
    ws["A24"].font = F_SECTION
    ws["A25"] = "领域＼维度"
    for j, d in enumerate(DIMS, start=2):
        ws.cell(row=25, column=j, value=DIM_LABELS[d])
    ws.cell(row=25, column=7, value="行合计")
    style_header(ws, 25, range(1, 8), height=20)
    ...
    for i, (dom, wmap) in enumerate(weight_rows, start=26):
        ...
        for j, d in enumerate(DIMS, start=2):
            ...
            cell.alignment = CENTER
            cell.border = BORDER
        sc = ws.cell(row=i, column=7, value=f"=SUM(B{i}:F{i})")
        sc.number_format = "0%"
        sc.alignment = CENTER
        sc.border = BORDER
        ws.conditional_formatting.add(
            f"G{i}", CellIsRule(operator="notEqual", formula=["1"],
                                fill=PatternFill("solid", fgColor="C00000"),
                                stopIfTrue=True))
    set_widths(ws, {"A": 26, "B": 10, "C": 10, "D": 28, "E": 12,
                    "F": 10, "G": 8, "H": 3, "I": 16})
...
def build_anchor_sheet(wb):
    ...
    ws["A1"] = "评分锚点对照表 —— 打分时逐档对照，并在登记册 W 列填写对应依据"
    ws["A1"].font = F_TITLE
    ...
    blocks = [
        ("发生可能性（1-5）", [
            ...
            (4, "本企业年内已发生，或同行业高发", "ACFE：组织年均约 5% 收入流失于舞弊"),
            (5, "已多次发生或正在发生，相关控制基本失效", "—"),
        ]),
        ("经济损失 —— 国资委资产损失分级 + 国务院令第 493 号", [
            (1, "<100 万元：未达“一般资产损失”标准，企业内部可消化", "国资委办法（一般以下）"),
            (2, "100~500 万元：构成“一般资产损失”", "国资委办法：<500 万为一般"),
            (3, "500~5000 万元：构成“较大资产损失”；493 号令“较大事故”直接损失落于本档", "国资委办法；国务院令第 493 号"),
            (4, "5000 万~1 亿元：构成“重大资产损失”；493 号令“重大事故”量级", "国资委办法；493 号令"),
            (5, "≥1 亿元：493 号令“特别重大事故”量级，危及企业生存", "493 号令第三条"),
        ]),
        ("合规法律 —— 个保法/数安法/GDPR + 国资委不良后果三档", [
            (1, "内部制度瑕疵，责令整改即可，无外部后果", "—"),
            (2, "监管关注/责令限期整改；罚款 <100 万元", "个保法第 66 条第一款"),
            (3, "一般行政处罚，罚款 100~1000 万元；责任人被处分", "数安法第 45 条；网数条例第 56 条"),
            (4, "情节严重：罚款 1000~5000 万或上年营业额 5%（GDPR 顶格 2000 万欧/4% 同档）", "个保法第 66 条第二款；GDPR Art.83"),
            (5, "刑事责任移送、责令停业/吊销执照；影响社会与国家层面", "国资委重大不良后果；刑法/监察法"),
        ]),
        ("运营中断 —— 493 号令事故等级 + ISO 22301 RTO 分级", [
            (1, "局部岗位受阻，当日恢复（RTO<8h），无人员伤害", "ISO 22301 恢复目标分级"),
            (2, "单部门中断，RTO 1~3 天；轻微伤害事件", "ISO 22301"),
            (3, "跨部门/跨区域中断，RTO 3~30 天；或 493 号令“一般事故”", "493 号令第三条"),
            (4, "核心业务瘫痪 1~6 个月；或“较大/重大事故”量级", "493 号令第三条"),
            (5, "集团级瘫痪超 6 个月；或“特别重大事故”（≥30 人死亡或≥1 亿元）", "493 号令第三条"),
        ]),
        ("声誉舆情 —— 国资委不良后果三档 × 传播层级", [
            (1, "无外部感知，仅内部知晓", "—"),
            (2, "个别投诉、本地媒体负面报道，影响限于涉事企业", "国资委：一般不良后果"),
            (3, "行业内流传、省级媒体或行业监管通报", "国资委：较大不良后果"),
            (4, "全国性媒体报道、上级通报批评，资本市场负面反应", "国资委：较大~重大不良后果"),
            (5, "国家级媒体/监管部门点名、社会舆论事件、引发行业整顿", "国资委：重大不良后果"),
        ]),
        ("舞弊风险 —— ACFE 2024 基准 + 监察机关职务犯罪管辖", [
            (1, "无舞弊诱因，职责分离健全且经穿行测试，无历史案例", "COSO 控制环境；ISO 37001"),
            (2, "一般诱因但控制可见；潜在损失 <100 万元", "ACFE 2024：中位损失 $14.5 万"),
            (3, "诱因集中（权限集中/监督薄弱）；潜在损失 100~500 万元", "ACFE 2024：75 分位 $75 万"),
            (4, "诱因高危或有历史案例；潜在损失 500~5000 万元；涉嫌职务违法", "国资委较大~重大资产损失档"),
            (5, "系统性舞弊土壤；潜在损失 ≥5000 万或 ≥年营收 5%", "ACFE：年均 5% 收入流失于舞弊"),
        ]),
    ]
    ...
    for title, items in blocks:
        ...
        row += 1
        for score, anchor, source in items:
            a = ws.cell(row=row, column=1, value=score)
            a.alignment = CENTER
            b = ws.cell(row=row, column=2, value=anchor)
            b.alignment = LEFT
            c = ws.cell(row=row, column=3, value=source)
            c.alignment = LEFT
            c.font = F_NOTE
            for cc in (a, b, c):
                cc.border = BORDER
...
def build_register_sheet(wb, risks):
    ...
    headers = ["风险编号", "风险名称", "所属领域", "风险描述", "责任部门", "评估期间",
               "发生可能性\n(1-5)", "经济损失\n(1-5)", "合规法律\n(1-5)", "运营中断\n(1-5)",
               "声誉舆情\n(1-5)", "舞弊风险\n(1-5)",
               "综合影响", "固有风险", "最弱控制分", "折减系数", "剩余风险", "剩余等级",
               "影响档\n(辅助)", "固有档\n(辅助)", "建议审计频次", "控制挽回率", "打分依据"]
    ws["A1"] = "风险登记册（黄色列录入，灰色列自动计算，请勿改写公式）"
    ws["A1"].font = F_TITLE
    ...
    for r_off, r in enumerate(risks):
        ...
        for c, v in enumerate(vals, start=1):
            ...
            if c <= 6 or (7 <= c <= 12):
                cell.fill = FILL_INPUT
        basis = ws.cell(row=row, column=23, value=r.get("rationale", ""))
    ...
    for row in range(4, REG_MAX + 1):
        formulas = {
            13: (f'=IF($A{row}="","",ROUND(MAX(SUMPRODUCT(H{row}:L{row},'
                 f'IFERROR(VLOOKUP($C{row},参数配置!$A$26:$F$38,{{2,3,4,5,6}},FALSE),'
                 f'VLOOKUP(参数配置!$A$26,参数配置!$A$26:$F$38,{{2,3,4,5,6}},FALSE))),'
                 f'参数配置!$E$13*MAX(H{row}:L{row})),2))'),
            14: f'=IF($A{row}="","",ROUND(G{row}*M{row},2))',
            15: (f'=IF($A{row}="","",IF(COUNTIFS(控制措施表!$B:$B,$A{row},控制措施表!$C:$C,$F{row},'
            ...
            16: f'=IF($A{row}="","",IF(N{row}="",0,VLOOKUP(N{row},参数配置!$D$4:$E$8,2,FALSE)))',
            17: f'=IF($A{row}="","",ROUND(O{row}*(1-P{row}),2))',
            18: f'=IF($A{row}="","",IF(Q{row}>=参数配置!$B$12,"极高",IF(Q{row}>=参数配置!$B$13,"高",IF(Q{row}>=参数配置!$B$14,"中",IF(Q{row}>=参数配置!$B$15,"低","极低")))))',
            19: f'=IF($A{row}="","",ROUND(M{row},0))',
            20: f'=IF($A{row}="","",ROUND(N{row},0))',
            21: f'=IF($A{row}="","",IF(R{row}="极高","每年必审",IF(R{row}="高","每年审计",IF(R{row}="中","两年一轮","按需抽查"))))',
            22: f'=IF($A{row}="","",IF(N{row}=0,"",(N{row}-Q{row})/N{row}))',
        }
            ...
            for j in range(5):
                ...
                if count_mode:
                    fml = (f'=COUNTIFS(风险登记册!$F$4:$F${REG_MAX},{crit},'
                           ...
                           f'风险登记册!$S$4:$S${REG_MAX},{col_letter}${head})')
                else:
                    fml = (f'=IFERROR(AVERAGEIFS(风险登记册!$Q$4:$Q${REG_MAX},'
...
def build_summary_sheet(wb):
    ...
    for i, dom in enumerate(DOMAINS, start=6):
        ...
        c = ws.cell(row=i, column=3, value=(
            f'=IFERROR(ROUND(AVERAGEIFS(风险登记册!$Q$4:$Q${REG_MAX},'
            f'风险登记册!$C$4:$C${REG_MAX},$A{i},'
            f'风险登记册!$F$4:$F${REG_MAX},{crit}),2),"—")'))
        d = ws.cell(row=i, column=4, value=(
            f'=IFERROR(MAXIFS(风险登记册!$Q$4:$Q${REG_MAX},'
            f'风险登记册!$C$4:$C${REG_MAX},$A{i},'
            f'风险登记册!$F$4:$F${REG_MAX},{crit}),"—")'))
        e = ws.cell(row=i, column=5, value=(
            f'=COUNTIFS(风险登记册!$R$4:$R${REG_MAX},"极高",'
            f'风险登记册!$C$4:$C${REG_MAX},$A{i},'
            f'风险登记册!$F$4:$F${REG_MAX},{crit})'))
        f_ = ws.cell(row=i, column=6, value=(
            f'=COUNTIFS(风险登记册!$R$4:$R${REG_MAX},"高",'
            f'风险登记册!$C$4:$C${REG_MAX},$A{i},'
            f'风险登记册!$F$4:$F${REG_MAX},{crit})'))
        g = ws.cell(row=i, column=7, value=(
            f'=IFERROR(AVERAGEIFS(风险登记册!$V$4:$V${REG_MAX},'
            f'风险登记册!$C$4:$C${REG_MAX},$A{i},'
            f'风险登记册!$F$4:$F${REG_MAX},{crit}),"—")'))
    ...
    reg_cols = {2: "A", 3: "B", 4: "C", 5: "E", 6: "G", 7: "N", 8: "O", 9: "Q", 10: "R"}
...
def build_readme_sheet(wb):
    ...
    lines = [
        ...
        ("", None),
        ("【模型】综合影响 = MAX(五维加权分, 一票否决系数×最高维度分)：任一维度打 4/5 分时，致命后果不被其他低分维度稀释。", None),
        ("　　风险领域 12 个、4 大类（战略与治理/财务/运营/合规与安全），对齐《企业内部控制应用指引》与《中央企业全面风险管理指引》。", None),
        ("　　五维权重按“所属领域”自动取用分领域权重矩阵（参数配置页 A24 起），未列出的领域用“全领域默认”行。", None),
        ("　　固有风险 = 可能性(1-5) × 综合影响；剩余风险 = 固有 × (1−折减系数)。", None),
        ("　　折减系数由该风险“关键控制点”的最低分查映射表确定（短板效应）；未标记关键控制时退回全部控制点取最低；无控制点则不折减。", None),
        ("　　控制挽回率 = (固有−剩余)/固有，衡量控制环境化解风险的比例，见登记册 V 列与汇总页领域表。", None),
        ...
        ("", None),
        ("【日常操作】1. 在「风险登记册」黄色列维护风险条目与六项打分（下拉限选 1-5、领域、期间）。", None),
