...
桌面端仅支持 Windows（Windows 10/11 x64），安装后不需要另装 Python，也不要求用户使用命令行。应用本身按用户安装；若系统缺少指定版本的 Microsoft Visual C++ Runtime，安装器会单独启动微软官方前置组件并请求 UAC。它把审计报告转换成可复核、可复用的结构化风险证据，再接入本项目现有的确定性评分、Excel、热力图和报告链路。

- 生产桌面端默认进入空的报告目录，不自动加载示例数据，也不显示“载入内置示例”。仓库内虚构测试数据仅用于非生产自动验收。
- 首版一个单主体信息目录严格对应一个被审计主体。目录产生报告信息后不能直接改换主体；集团组织树、跨主体选择和集团汇总图谱留待后续规划。
- 支持文字型 PDF、扫描 PDF 和 DOCX；不支持 `.doc`。处理顺序固定为：本地文本提取 → 自动 OCR → 可选视觉模型兜底 → 大模型判断。
- 添加报告时选择原文件、审计项目、模型和用于最小风险匹配的现行正式工作簿。模型只接收 `risk_id/name/domain/description`；责任部门、既有评分、控制和打分依据留在本地。
- 模型通过 OpenAI 兼容接口配置；API Key 存入 Windows 凭据管理器，不写入 SQLite、报告目录或日志。保存或修改配置后必须重新“测试连接”。
- 报告完成提取和人工确认后，只把报告元数据、关键来源摘录、页码、建议评分及复核结果写入用户指定目录；不会复制原始报告，也不保留完整提取正文和页面图片。上传日期是报告列表的主要日期，识别出的正式报告日期作为附加信息。
- 用户可按上传日期和审计项目选择一份或多份报告形成风险评估批次。不同报告指向同一风险时进入“相似发现处理”；推荐“为同一风险的两条证据”，保留全部报告、页码和事实依据，不删除原发现，也不平均建议评分。
- 批次中必须重新人工确认风险映射、可能性、八个影响维度、责任部门、当前控制和整改状态，现有 `tools/common.py` 才计算正式分数。写入前必须查看变更预览；输出为 `audit_risk_register_yyyyMMdd_HHmm.xlsx` 及对应导出目录，不覆盖原工作簿。
- 报告目录支持单条删除和“清空报告列表”；普通删除移入当前目录的回收站，不删除原始报告、历史批次或已生成工作簿。
- 不提供知识库、RAG、向量检索或报告问答。历史审计发现只能证明报告形成时的事实，不能单独证明当前剩余风险。

桌面端包含两个四步流程：报告进入目录（选择 → 提取 → 复核 → 保存结构化信息）和风险评估批次（选择报告 → 相似发现处理 → 确认当前控制 → 预览并生成版本化工作簿）。完整说明见[使用手册第 11 章](docs/使用手册.md#11-windows-桌面端审计报告风险评估)。
