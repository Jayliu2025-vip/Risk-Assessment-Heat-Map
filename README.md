...
桌面端仅支持 Windows（Windows 10/11 x64），安装后不需要另装 Python，也不要求用户使用命令行。应用本身按用户安装；若系统缺少指定版本的 Microsoft Visual C++ Runtime，安装器会单独启动微软官方前置组件并请求 UAC。它把过往审计报告转换成可复核的风险建议，再接入本项目现有的确定性评分、Excel、热力图和报告链路。

- 支持文字型 PDF、扫描 PDF 和 DOCX；不支持 `.doc`。处理顺序固定为：本地文本提取 → 自动 OCR → 可选视觉模型兜底 → 大模型判断。
- 第一步同时选择报告和现行正式工作簿；分析、风险匹配、预览和写入绑定同一工作簿及其哈希，工作簿变更后必须重新选择。模型只接收用于匹配的 `risk_id/name/domain/description`，责任部门、既有评分、控制和打分依据留在本地。
- 模型通过 OpenAI 兼容接口配置，可连接符合接口约定的云端或本地服务；API Key 存入 Windows 凭据管理器，不写入 SQLite 或日志。保存或修改配置后必须重新“测试连接”才能分析。使用云端模型时，提取文本、上述最小风险目录以及启用视觉兜底的页面图片会发送给所配置的服务方。
...
桌面端采用四步流程：选择报告、现行工作簿与模型 → 自动提取和分析 → 复核审计发现 → 预览并生成版本化工作簿。完整安装、配置、隐私和故障处理见[使用手册第 11 章](docs/使用手册.md#11-windows-桌面端审计报告风险评估)。
